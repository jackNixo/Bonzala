  ...
  private func generateQRCode(from string: String, size: CGSize) -> UIImage? {
        if let data = string.data(using: .utf8), let qrFilter = CIFilter(name: "CIQRCodeGenerator") {
            ...
            if let qrImage = qrFilter.outputImage {
                ...
                filter?.setValue(CIColor(red: 0/255, green: 35/255, blue: 29/255), forKey: "inputColor0") // Цвет фона
                filter?.setValue(CIColor(red: 0/255, green: 136/255, blue: 87/255), forKey: "inputColor1") //  Цвет QR-кода
